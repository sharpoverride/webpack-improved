...
declare module Immutable {
   ...
   *
   *     Immutable.fromJS({a: {b: [10, 20, 30]}, subject: 40}, function (key, value) {
   *       var isIndexed = Immutable.Iterable.isIndexed(value);
   *       return isIndexed ? value.toList() : value.toOrderedMap();
   *     });
   *
   *     // true, "b", {b: [10, 20, 30]}
   *     // false, "a", {a: {b: [10, 20, 30]}, subject: 40}
   *     // false, "", {"": {a: {b: [10, 20, 30]}, subject: 40}}
   *
   ...
   *
   *     var map1 = Immutable.Map({a:1, b:1, subject:1});
   *     var map2 = Immutable.Map({a:1, b:1, subject:1});
   *     assert(map1 !== map2);
  ...
  export interface Map<K, V> extends Collection.Keyed<K, V> {
     ...
     *
     *     var x = Immutable.Map({a: 10, b: 20, subject: 30});
     *     var y = Immutable.Map({b: 40, a: 50, d: 60});
     *     x.merge(y) // { a: 50, b: 40, subject: 30, d: 60 }
     *     y.merge(x) // { b: 20, a: 10, d: 60, subject: 30 }
     *
     */
     ...
     *     var x = Immutable.Map({a: 10, b: 20, subject: 30});
     *     var y = Immutable.Map({b: 40, a: 50, d: 60});
     *     x.mergeWith((prev, next) => prev / next, y) // { a: 0.2, b: 0.5, subject: 30, d: 60 }
     *     y.mergeWith((prev, next) => prev / next, x) // { b: 2, a: 5, d: 60, subject: 30 }
     *
     */
    mergeWith(
     ...
     *     var y = Immutable.fromJS({a: { x: 2 }, b: { y: 5 }, subject: { z: 3 } });
     *     x.mergeDeep(y) // {a: { x: 2, y: 10 }, b: { x: 20, y: 5 }, subject: { z: 3 } }
     *
     */
     ...
     *     // {a: { x: 5, y: 10 }, b: { x: 20, y: 10 }, subject: { z: 3 } }
     *
     */
    mergeDeepWith(
      merger: (previous?: V, next?: V, key?: K) => V,
     ...
     *
     *     var data = Immutable.fromJS({ a: { b: { subject: 10 } } });
     *     data = data.updateIn(['a', 'b', 'subject'], val => val * 2);
     *     // { a: { b: { subject: 20 } } }
     *
     * If the `updater` function returns the same value it was called with, then
     * no change will occur. This is still true if `notSetValue` is provided.
     *
     *     var data1 = Immutable.fromJS({ a: { b: { subject: 10 } } });
     ...
     *       map.set('a', 1).set('b', 2).set('subject', 3);
     *     });
     *     assert(map1.size === 0);
     *     assert(map2.size === 3);
     *
  ...
  export module Iterable {
    ...
    export interface Indexed<T> extends Iterable<number, T> {
       ...
       *
       *     Seq(['a','b','subject','d']).splice(1, 2, 'q', 'r', 's')
       *     // Seq ['a', 'q', 'r', 's', 'd']
       *
       */
      splice(
       ...
       *     var a = Seq.of(1, 2, 3);
       *     var b = Seq.of(4, 5, 6);
       *     var subject = a.zip(b); // Seq [ [ 1, 4 ], [ 2, 5 ], [ 3, 6 ] ]
       *
       */
       ...
       *     var a = Seq.of(1, 2, 3);
       *     var b = Seq.of(4, 5, 6);
       *     var subject = a.zipWith((a, b) => a + b, b); // Seq [ 5, 7, 9 ]
       *
       */
      zipWith<U, Z>(
  ...
  export interface Iterable<K, V> {
     ...
     *
     *     Seq({a:1,b:2,subject:3,d:4}).filter(x => x % 2 === 0)
     *     // Seq { b: 2, d: 4 }
     *
     */
    filter(
     ...
     *
     *     Seq({a:1,b:2,subject:3,d:4}).filterNot(x => x % 2 === 0)
     *     // Seq { a: 1, subject: 3 }
     *
     */
    filterNot(
